...
const series = ref([]);
const labels = ref([]);
const colors = ['#d30476', '#9d9906', '#188a8d', '#0495c7', '#4d9b01', '#ae098b'];
...
onMounted(() => {
    axios.post(route('dashboard.stages-pie-chart')).then(response => {
        series.value = response.data.chart.series;
        labels.value = response.data.chart.labels;
...
<template>
    ...
    >
        <div class="mb-3 justify-between gap-4 sm:flex">
            <div>
                <h4 class="text-xl font-bold text-black dark:text-white">Stages Status</h4>
            </div>
            <div>
...
<!--                </div>-->
        ...
        <div class="-mx-8 flex flex-wrap gap-y-3">
            <div
                v-for="(label, index, index2) in labels"
                :key="index"
                class="w-full px-8 sm:w-1/2">
                <div class="flex w-full items-center">
                    <span class="mr-2 block h-3 w-full max-w-3 rounded-full" :style="{'background-color' : colors[index2]}"></span>
                    <p class="flex w-full justify-between text-sm font-medium text-black dark:text-white">
                        <span>{{label}}</span>
                        <span>{{series[index2]}}</span>
                    </p>
                </div>
            </div>
        </div>
